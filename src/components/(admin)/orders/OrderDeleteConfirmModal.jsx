...
export default function OrderDeleteConfirmModal({
  isOpen,
  order,
  deleting,
  onClose,
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[95] bg-[#141810]/55 backdrop-blur-sm"
        onClick={onClose}
      />
        ...
        <div className="w-full max-w-lg overflow-hidden rounded-[32px] border border-white/70 bg-[#fffaf9] shadow-[0_40px_120px_-40px_rgba(20,24,16,0.45)]">
          ...
          <div className="space-y-5 p-6 sm:p-8">
            <div className="rounded-[24px] border border-[#f6ddd7] bg-[#fff3f0] px-5 py-4">
              ...
              <p className="mt-2 text-sm text-[#7b6f6b]">{order.name}</p>
              {order.product ? (
                <p className="mt-1 text-xs text-[#7b6f6b]">{order.product}</p>
              ) : null}
            </div>
