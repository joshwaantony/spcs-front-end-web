...
export default function DeleteOfferModal({ isOpen, onClose, offer }) {
  const {
    deleting,
    deleteError,
    deleteSuccess,
  ...
  useEffect(() => {
    ...
    resetDeleteOfferState();
  }, [deleteSuccess, resetDeleteOfferState, showToast]);
  ...
  const handleDelete = async () => {
    try {
      ...
      const remainingTotal = Math.max(0, (total || 0) - 1);
      const lastValidPage = Math.max(1, Math.ceil(remainingTotal / limit));
      const nextPage = Math.min(page, lastValidPage);
      await fetchOffers(nextPage, limit);
      handleClose();
    } catch (error) {
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          ...
          <button
            ...
            aria-label="Close delete offer popup"
          >
        ...
        <div className="rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-gray-700">
          ...
          <span className="font-bold text-charcoal">"{offer.title}"</span>?
        </div>
